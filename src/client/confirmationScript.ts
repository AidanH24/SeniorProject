...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (storedSelectedServices) {
        ...
        if (servicesElement) servicesElement.textContent = selectedServices.join(', ');
    }
    ...
    if (storedAppointmentData) {
        ...
        if (dateElement) dateElement.textContent = `${appointmentData.dayOfWeek}, ${appointmentData.month} ${appointmentData.day}`;
...
async function finish() {
    ...
    const getText = (id: string): string => {
        ...
        if (!el) return "";
        if ("value" in el) return (el as HTMLInputElement).value.trim();
